...
import { HierarchicalRegionSelect } from './HierarchicalRegionSelect'
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          {Object.keys(hierarchy).length === 0 && availableSegments.length === 0 ? (
            ...
            // Special hierarchical region selector for "By Region" segment type
            <>
              <HierarchicalRegionSelect
                availableSegments={availableSegments}
                onSelect={(segment) => {
                  setCurrentSegmentSelection(segment)
                  // Auto-add the segment
                  const id = `${selectedSegmentType}::${segment}`
                  const exists = selectedSegments.find(s => s.segment === segment && s.type === selectedSegmentType)
